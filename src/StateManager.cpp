...
void StateManager::SetLoopStart() {
  m_loopStartTime = millis();
}

void StateManager::SetLoopEnd() {
  m_loopCount++;
  word currentLoopTime = millis() - m_loopStartTime;
  ...
  if (millis() > m_loopMeasureStart + 10000) {
...
void StateManager::Update() {
  ...
  m_values.Put("ChipID", String(ESP.getChipId()));
  ...
  m_values.Put("LD.Min", String(m_loopDurationMin));
  m_values.Put("LD.Avg", String(m_loopDurationAvg));
  m_values.Put("LD.Max", String(m_loopDurationMax));
...
String StateManager::GetKVP(uint interval) {
  String result = "";
