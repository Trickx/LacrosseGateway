...
bool Nextion::Begin(ESP8266WebServer *webServer, ESP8266SoftSerial *softSerial, unsigned long baud, bool addUnits) {
  m_server = webServer;
...
void Nextion::Handle(WSBase::Frame frame, unsigned int framesPerMinute, String version, int32_t rssi, bool fhemIsConnected, bool bridge1Connected, bool bridge2Connected) {
  if(millis() < m_lastUpdate) {
    m_lastUpdate = millis();
  }
  
  if(millis() > m_lastUpdate + 2000) {
    SendCommand("LGW#rssi.txt=\"" + String(rssi) + " dBm\"");
    ...
    SendCommand("LGW#pres.txt=\"" + String(frame.Pressure) + (m_addUnits ? " hPa\"": "\""));
    SendCommand("LGW#ip.txt=\"" + WiFi.localIP().toString() + "\"");
    SendCommand("LGW#fpm.txt=\"" + String(framesPerMinute) + "\"");
